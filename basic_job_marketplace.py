...
PORT = int(os.environ.get("PORT", 8086))  # FIX: Convert to int
...
# Find Job Providers - FIXED INDICES
...
async def find_job_providers(
    ...
    try:
        ...
        for i, provider in enumerate(providers, 1):
            ...
            result += f"   Location: {provider[3]}, {provider[4]}\n"  # location (5), city (6)
            result += f"   Experience: {provider[5]}\n"  # experience (index 7)
            result += f"   Rate: {provider[6]}\n\n"  # rate (index 8)
...
Phone: {provider[1]}
Services: {provider[2]}
Location: {provider[3]}, {provider[4]}
Experience: {provider[5]}
Rate: {provider[6]}
Status: {'Available' if provider[8] else 'Not Available'}
...
# Browse All Providers - FIXED INDICES
...
async def browse_job_providers(
    ...
    try:
        ...
        for i, provider in enumerate(providers, 1):
            ...
            result += f"   Services: {provider[2]}\n"  # skills (index 4)
            result += f"   Location: {provider[3]}, {provider[4]}\n"  # location (5), city (6)
            result += f"   Experience: {provider[5]} | Rate: {provider[6]}\n\n"  # experience (7), rate (8)
...
async def job_marketplace_stats() -> str:
    ...
    try:
        ...
        cursor.execute('SELECT COUNT(*) FROM job_providers WHERE available=1')
        available_providers = cursor.fetchone()
        
        cursor.execute('SELECT COUNT(*) FROM job_seekers')
        total_seekers = cursor.fetchone()
        ...
        top_services = sorted(service_count.items(), key=lambda x: x[9], reverse=True)[:5]
